 ...
 * @since 3.2.0
 */
public class GeoFence_Round_Activity extends CheckPermissionsActivity implements OnClickListener, GeoFenceListener, OnMapClickListener, LocationSource, AMapLocationListener, OnCheckedChangeListener {
  ...
  @Override public void onClick(View v) {
    switch (v.getId()) {
      case R.id.bt_addFence:
        ...
        break;
      case R.id.bt_option:
        if (btOption.getText().toString().equals(getString(R.string.showOption))) {
  ...
  private void drawCircle(GeoFence fence) {
    ...
    // 绘制一个圆形
    mAMap.addCircle(new CircleOptions().center(center).radius(fence.getRadius()).strokeColor(Const.STROKE_COLOR).fillColor(Const.FILL_COLOR).strokeWidth(Const.STROKE_WIDTH));
  ...
  private void drawPolygon(GeoFence fence) {
    ...
    for (List<DPoint> subList : pointList) {
      ...
      polygonOption.strokeColor(Const.STROKE_COLOR).fillColor(Const.FILL_COLOR).strokeWidth(Const.STROKE_WIDTH);
  ...
  @Override public void onGeoFenceCreateFinished(final List<GeoFence> geoFenceList, int errorCode, String customId) {
    ...
    switch (buttonView.getId()) {
      case R.id.cb_alertIn:
        if (isChecked) {
          ...
          activatesAction = activatesAction & (GeoFenceClient.GEOFENCE_OUT | GeoFenceClient.GEOFENCE_STAYED);
        }
        break;
      case R.id.cb_alertOut:
        if (isChecked) {
          ...
          activatesAction = activatesAction & (GeoFenceClient.GEOFENCE_IN | GeoFenceClient.GEOFENCE_STAYED);
        }
        break;
      case R.id.cb_alertStated:
        if (isChecked) {
          ...
          activatesAction = activatesAction & (GeoFenceClient.GEOFENCE_IN | GeoFenceClient.GEOFENCE_OUT);
        }
        break;
      default:
        break;
